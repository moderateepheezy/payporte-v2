...
open class Stepperier: UIControl {
    ...
    internal func commonInitalization() {
        ...
        subtractionSymbolControl.addTarget(self, action: #selector(didTapSubtractionSymbol(button:)), for: .touchUpInside)
        ...
        additionSymbolControl.addTarget(self, action: #selector(didTapAdditionSymbol(button:)), for: .touchUpInside)
    ...
    internal func didTapAdditionSymbol(button: CenteredLineSymbolControl) {
        DispatchQueue.main.async{
            guard self.isOperationSymbolsManualClicksEnabled && self.value < self.maximumValue else { return }
            self.updateValueWithEvents(self.value + 1)
        }
    }
    
    internal func didTapSubtractionSymbol(button: CenteredLineSymbolControl) {
        DispatchQueue.main.async{
            guard self.isOperationSymbolsManualClicksEnabled && self.value > self.minimumValue else { return }
            self.updateValueWithEvents(self.value - 1)
        }

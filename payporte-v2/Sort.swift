...
public final class Sort: NSCoding {
  ...
  public required init(json: JSON) {
    ...
    if let items = json[SerializationKeys.filter].array { filter = items.map { SortFilter(json: $0) } }
  }
  ...
  required public init(coder aDecoder: NSCoder) {
    ...
    self.filter = aDecoder.decodeObject(forKey: SerializationKeys.filter) as? [SortFilter]
  }
